...
def color_iter(name, n):
    if name in sequential:
        color_sets = sequential[name][n]
    elif name in diverging:
        color_sets = diverging[name][n]
    elif name in qualitative:
        color_sets = qualitative[name][n]
    else:
        raise ValueError('Unknown color palette: %s' % name)
    color_array = np.array(color_sets)/255
    color_list = color_array.tolist()
    return iter(color_list)
