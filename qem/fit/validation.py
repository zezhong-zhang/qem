...
class FitterValidator:
    ...
    def validate_image(image: np.ndarray, max_size: int = 5000, max_memory_mb: int = 1000) -> np.ndarray:
        ...
        if image.dtype.type not in supported_dtypes:
            raise _invalid(
                ...
                "Convert using image.astype(np.float32)"
            )
    ...
    @staticmethod
    def validate_elements(elements: list[str] | None, required_length: int | None = None) -> list[str]:
        """
    ...
    def validate_coordinates(coordinates: np.ndarray, image_shape: tuple[int, int],
        ...
        if coordinates.shape[1] != 2:
            raise _invalid(
                ...
                "Reshape your array to (N, 2) where N is the number of atoms"
            )
