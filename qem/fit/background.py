...
class Background:
    ...
    def __init__(self, image: np.ndarray, dx: float = 1.0, mask: np.ndarray | None = None):
        """
    ...
    def _estimate_photutils_2d(self,
        ...
        # photutils Background2D raises ValueError for bad box/filter sizes
        # or mask shape; IndexError on degenerate meshes. RuntimeError guards
        # the rare numerical estimator failure.
        except (ValueError, IndexError, RuntimeError) as e:
            logging.warning(
                "Photutils failed (%s): %s, using median fallback",
                type(e).__name__, e,
            )
            return self._estimate_median_2d()
    ...
    def get_background_for_linear_estimation(self) -> float | np.ndarray:
        """
