...
def time_it(func):
    def wrapper(*args, **kwargs):
        ...
        print(f"Method {func.__name__!r} executed in {(end_time - start_time):.4f} seconds")
        return result
    return wrapper
...
class Benchmark:
    def __init__(self, filepath):
        ...
        try:
            ...
            if 'output' in legacyStatSTEM.keys():
                ...
                if 'volumes' in outputStatSTEM.keys() or 'Volumes' in outputStatSTEM.keys():
            ...
            raise ValueError("InputStatSTEM dictionary does not have correct keys in the input file.")

    @time_it
    def refine(self, atom_size = 0.5, tol = 1e-2, maxiter = 100, step_size = 1e-2, num_epoch = 10, batch_size = 1000, verbose = False, plot = True, guess_radius = False):
    ...
    def compare_residual(self, mode = 'both', folder_path=None, file_path=None, save=False):
        image = self.image
        if mode == 'StatSTEM':
    ...
    def compare_scs_map(self,folder_path=None, file_path=None, save=False):
        ...
        index_statstem_in_qem = np.array([np.argmin(np.sqrt((pos_x_statstem - x)**2 + (pos_y_statstem - y)**2)) for x,y in zip(pos_x, pos_y)])
